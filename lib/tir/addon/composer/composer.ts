...
import { type Expectation, type NodeListField, type ObjectField, type QueryField } from "./types";
...
class Composer {
  ...
  declare private readonly expectations: Map<string, Expectation>;

  declare private lastOperationIndex: number;
  declare private lastVariableIndex: number;

  declare private readonly operationVariables: Map<string, Set<ReturnType<ReturnType<typeof withScalar>>>>

  declare operations: Map<string, {
  ...
  public resolveFields = (
    ...
    const __path = key === 'edges' || __edges || __list ? [...path, responseKey, '#'] : [...path, responseKey,]
    ...
    switch (true) {
      // connection
      case __modelName !== undefined && (key.includes('Connection') || key.includes('connection') || __connection === true):
        expectationType = RootType.connection;
        break;
      // flat node list
      case (__modelName !== undefined || enforceModelName !== undefined) && __list === true:
      ...
      case (__modelName !== undefined || enforceModelName !== undefined) && key === 'edges' || __edges:
        expectationType = RootType.edges;
        break;
      // node
      case (__modelName !== undefined || enforceModelName !== undefined) && key === 'node' || key.includes('Node') || key.includes('node') || __node === true:
      ...
      case __modelName === undefined && enforceModelName === undefined && (fields || __queryParams || __scalars):
        expectationType = RootType.record;
        break;
      // scalar field of node
      case (__modelName !== undefined || enforceModelName !== undefined) && (fields === undefined && __queryParams === undefined || __scalars === undefined):
    ...
    // TODO: add directives
    // resolve queryParams
    if (Object.keys(RootType).filter(k => ![RootType.end, RootType.scalar].includes(k as RootType)).includes(expectationType)) {
      const params = __queryParams ? `(${this.handleVariables(__queryParams, uniqueOperationName)})` : '';
      return `${this.resolveFieldNaming(key, __alias, __modelName)} ${params} {
        ${Object.entries(fields as Record<string, QueryField>).forEach(([fieldName, declaration]) => {
        return this.resolveFields(
          ...
          __path,
          level + 1,
          [RootType.connection, RootType.edges, RootType.nodeList].includes(expectationType) ? __modelName : undefined
        )
      })}
      }`;
    } else {
  ...
  private getNextVariableIndex = () => {
    return this.lastVariableIndex += 1;
  };

  public getNextOperationIndex = () => {
    return this.lastOperationIndex += 1;
  };


  private resolveFieldNaming = (key: string, alias?: string, modelName?: keyof PodRegistry) => {
    ...
    if (modelName) {
      ...
      const maybeField = maybeProperty ? TYPE.META.fields[maybeProperty] : undefined;
      if (maybeProperty && maybeProperty !== key && maybeField && maybeField.alias) {
        // return aliased
  ...
  public handleVariables = (params: Record<string, ReturnType<typeof withScalar>>, uniqueOperationName: string) => {
    ...
    Object.entries(params).forEach(([key, configurator]) => {
      const variableName = `${key}${this.getNextVariableIndex()}`
  ...
  public composeOperationVariables = (operationName: string) => {
    ...
    return variables && variables.size !== 0 ? `(${Array.from(variables).map(item => `$${item.variableName}:${item.scalarType}`).join(', ')})` : '';
  };
