...
function belongsTo(
  ...
  assert(
    `An explicit modelName must be provided as first argument to "${belongsTo.prototype.name}".`,
  ...
  return function (
    ...
    assert(
      `${ERROR_MESSAGE_PREFIX}Decorator ${belongsTo.prototype.name} can only be applied to "string" properties`,
