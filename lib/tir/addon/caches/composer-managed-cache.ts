...
import type { ListRoot } from './list-root';
...
class ComposerManagedCache extends TirCache {
  ...
  private serialize = (
    ...
    if (
      ...
      if (expectation && expectation.responseKey === key) {
        const { type, modelName, queryParams, responseKey, identifierField } =
        ...
        switch (true) {
          ...
          case type === RootType.connection:
            ...
            const connectionRoot = this.getRoot(
              connectionRef,
            ) as ConnectionRoot<Pod>;
            if (__errors) {
              __errors.forEach((err) => connectionRoot.addError(err.message));
            } else {
              const __addables = new Set<ClientId>();
              Object.entries(value as RelayConnectionData).forEach(
              ...
              const connection = connectionRoot.getConnection(
                queryParams ?? {},
              );
          ...
          case type === RootType.nodeList:
            ...
            const listRoot = this.getRoot(ref) as ListRoot<Pod>;
            if (__errors) {
  ...
  private findResponseErrors = (
    errors: GraphQlErrorData[],
    path: (string | number)[],
  ) => {
    return (
      errors.filter((error) => error.path! && comparePaths(error.path, path)) ??
      null
    );
  };
}
