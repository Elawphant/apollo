...
module.exports = {
  ...
  read(path) {
    const schema = fs.readFile(path, 'utf8', (err, data) => {
      ...
      const query = types.find(
        (type) => type.kind === 'OBJECT' && type.name === 'Query',
      );
      const mutation = types.find(
        (type) => type.kind === 'OBJECT' && type.name === 'Mutation',
      );
      if (query) {
        ...
        TYPE?.fields?.forEach((field) => {
          if (field.type.ofType.kind === 'SCALAR') {
            podFields += `@attr() declare ${field.name}: ${
              ['String', 'Number', 'Boolean', 'Int', 'Float', 'Date'].includes(
      ...
      if (types) {
        types.forEach((type) => {
          ...
          type.fields.forEach((field) => {
            const fieldType =
              type.kind === 'NON_NULL' ? type.ofType.name : type.name;
            if (
              ['String', 'Int', 'Float', 'Boolean', 'ID', 'OBJECT'].includes(
                fieldType,
              ) &&
              !field.name
            ) {
            ...
            const config = {
              ...
              defaultNodeQuery: `
                    query ${name}NodeQuery ($id: ID!) {
                      ${camelizedName}__${classifiedName}${
                        NAMING_CONVENTIONS.item
                      }: node (id: $id) {
